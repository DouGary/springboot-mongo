...
@Component
public class Monitor4ASC815_22_999992 {
	@Autowired
	...
	Logger logger = LogManager.getLogger(Monitor4ASC815_22_999992.class);
	...
	public List<StockCode> getFinal(String endDateStr) throws Exception
	{
		...
		monitors.add("88781");
		monitors.add("88782");
		List<Scpool> scLst = scpoolRepository.findAllScpoolGteEndDateStrAndSellAllOut(endDateStr,endDateStr,true,monitors,1);
		...
		for (Scpool scpool : scLst) {
			if(resultMap.get(scpool.getName()+" "+scpool.getCode())==null){
				resultMap.put(scpool.getName()+" "+scpool.getCode(),1);
				resultMapWithMonitor.put(scpool.getName()+" "+scpool.getCode(),scpool.getMonitor());
			} else {
				int val = resultMap.get(scpool.getName()+" "+scpool.getCode());
				val++;
				resultMap.put(scpool.getName()+" "+scpool.getCode(),val);

				String oriMonitor = resultMapWithMonitor.get(scpool.getName()+" "+scpool.getCode());
				resultMapWithMonitor.put(scpool.getName()+" "+scpool.getCode(),  oriMonitor+" | "+scpool.getMonitor());
			}
...
//			}
			System.out.println(mapping.getKey() + "   策略个数:" + mapping.getValue()+"   策略:"+resultMapWithMonitor.get(mapping.getKey()) );
